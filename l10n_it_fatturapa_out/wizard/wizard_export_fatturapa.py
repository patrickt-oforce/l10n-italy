...
class WizardExportFatturapa(models.TransientModel):
    ...
    def _setIdTrasmittenteByCustomSender(self, sender, fatturapa):

        if not sender.country_id:
            raise UserError(
                _('Sender Country not set.'))
        IdPaese = sender.country_id.code

        IdCodice = sender.fiscalcode
        if not IdCodice:
            if sender.vat:
                IdCodice = sender.vat[2:]
        if not IdCodice:
            raise UserError(
                _('Sender does not have fiscal code or VAT number.'))
