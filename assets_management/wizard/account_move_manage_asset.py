...
class WizardAccountMoveManageAsset(models.TransientModel):
    ...
    def link_asset(self):
        ...
        if not method:
            raise ValidationError(
                _("Could not determine how to link move lines to asset"
    ...
    def check_pre_create_asset(self):
        ...
        if not self.move_line_ids:
            raise ValidationError(
                _("At least one move line is mandatory to create"
                  " a new asset!")
            )

        if not len(self.move_line_ids.mapped('move_id')) == 1:
            raise ValidationError(
                _("Cannot create asset if move lines come from different"
                  " account moves!")
            )
        ...
        if not all([
            ...
            raise ValidationError(
                _("You need to choose move lines with account `{}`"
    ...
    def check_pre_dismiss_asset(self):
        ...
        if not self.move_line_ids:
            raise ValidationError(
                _("At least one move line is mandatory to dismiss"
                  " an asset!")
            )

        if not len(self.move_line_ids.mapped('move_id')) == 1:
            raise ValidationError(
                _("Cannot dismiss asset if move lines come from different"
                  " account moves!")
            )
    ...
    def check_pre_link_asset(self):
        ...
        if len(self.move_line_ids.mapped('account_id')) > 1:
            raise ValidationError(
                _("Every move line must share the same account!")
            )
    ...
    def check_pre_update_asset(self):
        ...
        if not self.move_line_ids:
            raise ValidationError(
                _("At least one move line is mandatory to update"
                  " an asset!")
            )
    ...
    def get_update_asset_vals(self):
        ...
        for dep in asset.depreciation_ids.filtered(
            ...
            for move, lines in grouped_move_lines.items():
                ...
                move_type = self.get_move_journal_type_2_dep_line_type() \
                    .get(move_journal_type)
